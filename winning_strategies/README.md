...
> **FALSIFIED** (the alt's own momentum gates better than BTC ⇒ no cross-asset content — `experiments/validate_b_riskfilter.py`
...
- **Tier 3 — PRE-REGISTERED CANDIDATE (real signal, OOS proof pending).** A genuinely
  ...
  → **Over-round-gated favorite-tail** (`overround-gate.md`, NEW 2026-06-25) — gate favorite-tail (ask 0.95–0.99)
     ...
     this). Lifts fav-tail −0.006→+0.0065 but loss-light (13 losers) + fee-capped. `experiments/experiment_overround_gate.py`.
  ...
  → **B risk-filter** — ~~BTC-opposing skip on favorite-tail~~ **FALSIFIED 2026-06-25** (the alt's OWN 15s
     ...
     BTC lead). See `experiments/validate_b_riskfilter.py` CHECK 5. **Demoted off the candidate board.**
...
| ~~B risk-filter (skip BTC-opposing favorites)~~ | ~~3~~ → **FALSIFIED** | own-momentum gates better than BTC ⇒ no cross-asset content (`experiments/validate_b_riskfilter.py` CHECK 5) | `experiments/validate_b_riskfilter.py` |
| ~~Spike-gated fade~~ | ~~3~~ → **DEAD** | no dose-response across z + falling-knife fail; n=18 was best-of-cell noise | `experiments/experiment_spike_fade.py` |
