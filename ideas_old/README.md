...
| [Settlement-convergence lag](settlement-lag.md) — `experiment_settlement_lag.py` | DEAD — looked +EV (CI excl 0) but is a margin cherry-pick (ungated −0.0015, deflated p=0.89) and BASIS-flip dominated: favorite on Binance, settles on Chainlink; Pyth disagrees 2.79% > the 1.24% the edge needs | a **Chainlink price adapter** replaces the Binance proxy (removes the basis-flip tail) |
